...
<div id="wrapper">
    ...
    <div id="page-wrapper" >
        <div class="row">
            <div class="col-lg-12">
                <h3 class="page-header">常见问题管理</h3>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-12 blockquote-reverse">
                <a href="/admin/problem/toadd" class="btn btn-primary btn-sm">添加常见问题</a>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-12">
                    ...
                    <table class="table">
                        ...
                        <c:forEach items="${data.data}" var="item">
                            <tr>
                                ...
                                <td>${item.answer}</td>
                                <td class="text-right">
                                    <a class="btn btn-primary btn-xs" href="admin/problem/toupdate/${item.id}"><i class="fa fa-edit "></i> 编辑</a>
                                    <a class="btn btn-danger btn-xs" href="admin/problem/${item.id}/change?pager.offset=${p.offset}&delete=true" onclick="if(confirm('确定删除?')==false)return false;"><i class="fa fa-pencil"></i> 删除</a>
                                    <a class="btn btn-primary btn-xs" href="admin/problem/${item.id}/change?pager.offset=${p.offset}&status=${item.status==0}"> ${item.status==0?"发布":"取消发布"}</a>
                                    <a class="btn btn-primary btn-xs" href="admin/problem/lan/list/${item.id}"> 多语言</a>
                                </td>
                            </tr>
                        </c:forEach>
                        </tbody>
